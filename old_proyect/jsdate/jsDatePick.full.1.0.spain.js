...
*/
/*
	Adaptado a España por http://mooontes.com
*/
...
JsDatePick = function(configurationObject){
	...
	this.monthsTextualRepresentation = new Array("Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre");
...
JsDatePick.prototype.setConfiguration = function(aConf){
	...
	this.oConfiguration.id    			= (aConf["id"] != null) ? aConf["id"] : null;
	this.oConfiguration.path   			= (aConf["path"] != null) ? aConf["path"] : "img/";
...
JsDatePick.prototype.makeCalendar = function(){
	...
	JsDatePickBox.className = "JsDatePickBox";
	clearfix.className		  = "clearfix";
	closeButton.className	  = "jsDatePickCloseButton";
	closeButton.setAttribute("globalNumber",this.globalNumber);
	
	closeButton.onmouseover = function(){
		this.className = "jsDatePickCloseButtonOver";
		var globalRef = "gRef_"+this.getAttribute("globalNumber");
		eval(globalRef + ".setTooltipText(\"Cierra el calendario\");");
	};
	
	closeButton.onmouseout = function(){
		this.className = "jsDatePickCloseButton";
		var globalRef = "gRef_"+this.getAttribute("globalNumber");
		eval(globalRef + ".setTooltipText(\"\");");
	};
	
	closeButton.onmousedown = function(){
		this.className = "jsDatePickCloseButtonDown";
		var globalRef = "gRef_"+this.getAttribute("globalNumber");
		eval(globalRef + ".setTooltipText(\"Cierra el calendario\");");
	};
	...
	topCorner.className 	= "leftTopCorner";
	bottomCorner.className 	= "leftBottomCorner";
	wall.className 			= "leftWall";
	...
	topCorner.className 	= "rightTopCorner";
	bottomCorner.className 	= "rightBottomCorner";
	wall.className 			= "rightWall";
	...
	if (this.oConfiguration.isStripped){
		leftWall.className = "hiddenBoxLeftWall";
		rightWall.className = "hiddenBoxRightWall";
	} else {
		leftWall.className = "boxLeftWall";
		rightWall.className = "boxRightWall";
	}
	...
	if (this.oConfiguration.useMode == 2){
		if (this.oConfiguration.target != false){
			if (typeof($(this.oConfiguration.target)) != null){
				...
				if (this.oConfiguration.id != null){
					aSpan.setAttribute('id', this.oConfiguration.id);
				}
...
JsDatePick.prototype.getDOMCalendarStripped = function(){
	...
	if (this.oConfiguration.isStripped){
		boxMain.className = "boxMainStripped";
	} else {
	...
	boxMainInner.className 			= "boxMainInner";
	boxMainCellsContainer.className = "boxMainCellsContainer";
	tooltip.className 				= "tooltip";
	weekDaysRow.className			= "weekDaysRow";
...
JsDatePick.prototype.makeDOMWeekDays = function(aWeekDaysRow){
	var i=0;
	var d = document;
	//var weekDaysArray = new Array("Dom","Lun","Mar","Mie","Jue","Vie","Sab");
	var weekDaysArray = new Array("Lun","Mar","Mie","Jue","Vie","Sab","Dom");
	var textNode;
	var weekDay;
...
JsDatePick.prototype.populateMainBox = function(aMainBox){
	...
	var oDay 	= new Date(this.currentYear, cmpMonth, 1);
	var iStamp	= oDay.getTime();

	var oPath = this.oConfiguration.path;
	
	this.flag_DayMarkedBeforeRepopulation = false;
	this.setControlBarText(this.monthsTextualRepresentation[cmpMonth] + ", " + this.currentYear);
	
	var skipDays = parseInt(oDay.getDay()-1); //añadido -1 para q el primer día de la semana sea el Lunes
	var i=0;
	for (i=0; i<skipDays; i++){
		aDayDiv = d.createElement("div");
		aDayDiv.className = "skipDay";
	...
	while (oDay.getMonth() == cmpMonth){
		...
		aDayDiv.onmouseover = function(){
			...
			var oPath = eval(globalRef + ".getImagesPath()");
			...
			if (parseInt(this.getAttribute("isToday")) == 1){
				this.className = "dayOverToday";
				this.style.background = "url(" + oPath + currentColorScheme + "_dayOver.gif) left top no-repeat";
			} else {
		...
		aDayDiv.onmouseout = function(){
			...
			if (parseInt(this.getAttribute("isToday")) == 1){
				this.className = "dayNormalToday";
				this.style.background = "url(" + oPath + currentColorScheme + "_dayNormal.gif) left top no-repeat";
			} else {
				this.className = "dayNormal";
				this.style.background = "url(" + oPath + currentColorScheme + "_dayNormal.gif) left top no-repeat";
			}
		};
		...
		aDayDiv.onmousedown = function(){
			...
			if (parseInt(this.getAttribute("isToday")) == 1){
				this.className = "dayDownToday";
				this.style.background = "url(" + oPath + currentColorScheme + "_dayDown.gif) left top no-repeat";
			} else {
				this.className = "dayDown";
				this.style.background = "url(" + oPath + currentColorScheme + "_dayDown.gif) left top no-repeat";
			}
		};
		...
		if (this.isSelectedDay(oDay.getDate())){
			...
			if (parseInt(aDayDiv.getAttribute("isToday")) == 1){
				aDayDiv.className = "dayDownToday";
				aDayDiv.style.background = "url(" + oPath + this.oConfiguration.cellColorScheme + "_dayDown.gif) left top no-repeat";
			} else {
				aDayDiv.className = "dayDown";
				aDayDiv.style.background = "url(" + oPath + this.oConfiguration.cellColorScheme + "_dayDown.gif) left top no-repeat";
			}
			...
			if (parseInt(aDayDiv.getAttribute("isToday")) == 1){
				if (disabledDayFlag){
					...
					aDayDiv.style.background = "url(" + oPath + this.oConfiguration.cellColorScheme + "_dayNormal.gif) left top no-repeat";
				} else {
					aDayDiv.className = "dayNormalToday";
					aDayDiv.style.background = "url(" + oPath + this.oConfiguration.cellColorScheme + "_dayNormal.gif) left top no-repeat";
				}
			} else {
				if (disabledDayFlag){
...
JsDatePick.prototype.getSelectedDayFormatted = function(){
	if (this.flag_aDayWasSelected){
		...
		returnVal += this.selectedDayObject.year;
...
JsDatePick.prototype.setDaySelection = function(anElement){
	...
	if  (this.flag_DayMarkedBeforeRepopulation){
		...
		if (parseInt(this.lastMarkedDayObject.getAttribute("isToday")) == 1){
			this.lastMarkedDayObject.className = "dayNormalToday";
			this.lastMarkedDayObject.style.background = "url(" + this.oConfiguration.path + currentColorScheme + "_dayNormal.gif) left top no-repeat";
		} else {
			this.lastMarkedDayObject.className = "dayNormal";
			this.lastMarkedDayObject.style.background = "url(" + this.oConfiguration.path + currentColorScheme + "_dayNormal.gif) left top no-repeat";
		}
	}
	...
	if (parseInt(anElement.getAttribute("isToday")) == 1){
		anElement.className = "dayDownToday";
		anElement.style.background = "url(" + this.oConfiguration.path + currentColorScheme + "_dayDown.gif) left top no-repeat";
	} else {
		anElement.className = "dayDown";
		anElement.style.background = "url(" + this.oConfiguration.path + currentColorScheme + "_dayDown.gif) left top no-repeat";
	}
};
...
JsDatePick.prototype.getImagesPath = function(){
	return this.oConfiguration.path;
};
...
JsDatePick.prototype.getDOMControlBar = function(){
	...
	monthForwardButton.className 	= "monthForwardButton";
	monthBackwardButton.className 	= "monthBackwardButton";
	yearForwardButton.className 	= "yearForwardButton";
	yearBackwardButton.className 	= "yearBackwardButton";
	controlsBarText.className		= "controlsBarText";
	...
	monthForwardButton.onmouseover = function(){
		...
		this.className = "monthForwardButtonOver";
		...
		eval(globalRef + ".setTooltipText(\"Avanza un més\");");
	};
	...
	monthForwardButton.onmouseout = function(){
		...
		this.className = "monthForwardButton";
		var parentElement = this.parentNode;
	...
	monthForwardButton.onmousedown = function(){
		...
		this.className = "monthForwardButtonDown";
		var parentElement = this.parentNode;
	...
	monthBackwardButton.onmouseover = function(){
		...
		this.className = "monthBackwardButtonOver";
		var parentElement = this.parentNode;
		...
		var globalRef = "gRef_"+parentElement.getAttribute("globalNumber");
		eval(globalRef + ".setTooltipText(\"Retrocede un més\");");
	};
	...
	monthBackwardButton.onmouseout = function(){
		...
		this.className = "monthBackwardButton";
		var parentElement = this.parentNode;
	...
	monthBackwardButton.onmousedown = function(){
		...
		this.className = "monthBackwardButtonDown";
		var parentElement = this.parentNode;
	...
	yearForwardButton.onmouseover = function(){
		...
		this.className = "yearForwardButtonOver";
		var parentElement = this.parentNode;
		...
		var globalRef = "gRef_"+parentElement.getAttribute("globalNumber");
		eval(globalRef + ".setTooltipText(\"Avanza un año\");");
	};
	...
	yearForwardButton.onmouseout = function(){
		...
		this.className = "yearForwardButton";
		var parentElement = this.parentNode;
	...
	yearForwardButton.onmousedown = function(){
		...
		this.className = "yearForwardButtonDown";
		var parentElement = this.parentNode;
	...
	yearBackwardButton.onmouseover = function(){
		...
		this.className = "yearBackwardButtonOver";
		var parentElement = this.parentNode;
		...
		var globalRef = "gRef_"+parentElement.getAttribute("globalNumber");
		eval(globalRef + ".setTooltipText(\"Retrocede un año\");");
	};
	...
	yearBackwardButton.onmouseout = function(){
		...
		this.className = "yearBackwardButton";
		var parentElement = this.parentNode;
	...
	yearBackwardButton.onmousedown = function(){
		...
		this.className = "yearBackwardButtonDown";
		var parentElement = this.parentNode;
